...
% MODE = MODE_CLUSTER_K;
% 
% params = multiplane_params_example(9,45,[],[], 0.05);
% params.camera.focal = 2100;
% 
% if ~exist('wavefront_fn','var')
%     planes = multiplane_random_plane_generator( 2, [28]);
% end
% 
% 
% if exist('wavefront_fn','var')
...
% %[output_angles,fval] = fsolve(@(x) hinged_errorfunc( regions,plane_graph,boundary_pts,x,root_hypothesis),init_angles,options);
% output_angles = []
% output_angles
% 
...
for i=1:length(regions)
    drawtraj(regions(i).reconstructed,'',0,pcolours(i));
end
draw_camera_pyramid( final_params, tscale, tscale, 2*tscale )
