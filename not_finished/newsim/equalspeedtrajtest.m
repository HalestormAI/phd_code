...
if  ~exist('baseTraj','var')
    [baseTraj] = addTrajectoriesToPlane( basePlane, [], 1, 2000, 1, 0, 0 );
end
...
if ~exist( 'ERROR_SET', 'var' )
    ERROR_SET     = cell(size(PLANE_PARAMS,2),1);
    WE_HAVE_ERRORSET = 0;
else
    WE_HAVE_ERRORSET = 1;
end
...
save pretestdata basePlane baseTraj PLANE_PARAMS grid FOCAL
...
all_x0s    = cell( size(PLANE_PARAMS,2),1 );
all_xiters = cell( size(PLANE_PARAMS,2),1 );
all_exits  = cell( size(PLANE_PARAMS,2),1 );
...
for p=1:size(PLANE_PARAMS,2)
    ...
    tobeoptimised_x0 = x0s(SIDS(1:round(length(SIDS)*0.001)),:);
...
save pretestdata ERROR_SET -append
fid = fopen('progress.txt', 'a');
